...
describe('DataAnalyzerAgent', () => {
  ...
  describe('Data Completeness Check', () => {
    it('should calculate completeness score correctly', async () => {
      ...
      const completenessFinding = result.findings.find(f => f.description.includes('completeness'))
  ...
  describe('Improvement Suggestions', () => {
    it('should suggest data enrichment pipeline when quality issues exist', async () => {
      ...
      const enrichmentSuggestion = result.improvements.find(i => 
        i.title.includes('enrichment')
      )
    ...
    it('should suggest automated refresh for stale data', async () => {
      ...
      const refreshSuggestion = result.improvements.find(i => 
        i.title.includes('refresh')
      )
      ...
      expect(refreshSuggestion?.implementation?.validationCriteria).toContain('All health scores <7 days old')
    })
