...
describe('OptimizerAgent', () => {
  ...
  describe('Optimization Opportunities', () => {
    ...
    it('should detect high frequency of filter operations', async () => {
      mockContext.userActions = Array(150).fill({ type: 'filter', timestamp: new Date().toISOString(), details: {} })
    ...
    it('should not flag low filter operations', async () => {
      mockContext.userActions = Array(50).fill({ type: 'filter', timestamp: new Date().toISOString(), details: {} })
  ...
  describe('Improvement Suggestions', () => {
    ...
    it('should include implementation details for pagination', async () => {
      ...
      const paginationSuggestion = result.improvements.find(i => i.title.includes('pagination'))
      expect(paginationSuggestion?.implementation?.steps).toContain('Implement pagination component')
      expect(paginationSuggestion?.implementation?.validationCriteria).toContain('Page load time <500ms')
    })
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle empty user actions', async () => {
      ...
      expect(result).toBeDefined()
      expect(result.findings.filter(f => f.description.includes('filter operations'))).toHaveLength(0)
    })
  })
})
