...
export function useSafeKV<T>(key: string, initialValue: T): UseSafeKVReturn<T> {
  ...
  const [fallbackValue, setFallbackValue] = useState<T>(() => readFromStorage(key, initialValueRef.current))

  useEffect(() => {
    initialValueRef.current = initialValue
  }, [initialValue])

  const setFallback = useCallback<Dispatch<SetStateAction<T>>>(valueOrUpdater => {
